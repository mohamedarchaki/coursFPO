...
<body>
    ...
    $sqlStat=$pdo->prepare('SELECT * FROM cour WHERE id=?');
    $sqlStat->execute([$id]);
    $cour=$sqlStat->fetch(pdo::FETCH_ASSOC);
...
if(!empty($id_prof) && !empty($nom_cour) && !empty($semester) && !empty($filier)){
    ...
    require_once 'include/base_donner.php';
    $sqlStat=$pdo->prepare('UPDATE cour SET nom_cour=?, id_prof=?,semester=?,filier=?,file_name=?');
    ...
    <div class="row d-flex justify-content-center">
        <div class="col-xl-7 col-lg-8 col-md-4 col-11 text-center">
           <div class="card">
                <h5 class="text-center mb-4">Hello Mr </h5>
                ...
                <form class="form-card text-start" method="post" enctype="multipart/form-data">
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-sm-6 flex-column d-flex">
                             ...
                             <input type="text" name="nom_cour" value="<?= $cour['nom_cour']?>" placeholder="Enter le nom cour"> </div>
                        <div class="form-group col-sm-6 flex-column d-flex">
